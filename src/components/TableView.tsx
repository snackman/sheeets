    ...
    <>
      ...
      >
        ...
        <td
          ...
          className="px-3 py-1.5 text-xs font-semibold text-yellow-400/80 uppercase tracking-wider border-b border-blue-700/70"
        >
          {group.label}
        </td>
      </tr>
      ...
      {group.events.map((event) => {
        ...
        return (
          <tr
            key={event.id}
            className={`hover:bg-blue-900/70 transition-colors cursor-pointer ${event.isDuplicate ? 'bg-red-950/30' : event.isFeatured ? 'bg-yellow-500/5 border-l-2 border-l-orange-500/40' : 'bg-blue-950'}`}
              ...
              {(() => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    {fc > 0 && (
                      <span className="absolute -top-1 -right-1.5 min-w-[14px] h-[14px] flex items-center justify-center rounded-full bg-yellow-500 text-white text-[8px] font-bold px-0.5 pointer-events-none">
                        {fc}
                      </span>
                    )}
                  </button>
