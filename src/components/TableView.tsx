  ...
  return (
    <div className="max-w-full mx-auto px-2 sm:px-4 pb-3 flex-1 min-h-0 min-w-0 flex flex-col w-full">
      <div
        ...
        className="overflow-auto rounded-lg border border-violet-800 flex-1 min-h-0 min-w-0 overscroll-none"
      >
        <table className="w-full text-sm text-left md:table-fixed" style={{ minWidth: Math.max(640, 142 + tagsColWidth + 420) }}>
          ...
          <thead className="text-xs uppercase tracking-wider text-violet-300 bg-violet-900 border-b border-violet-800 sticky top-0 z-20">
            <tr>
              ...
              <th className="px-3 py-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={downloadCSV}
                      className="p-1 rounded text-violet-400 hover:text-violet-100 transition-colors cursor-pointer"
                      title="Download CSV"
                    ...
                    <a
                      ...
                      className="p-1 rounded text-violet-400 hover:text-violet-100 transition-colors"
    ...
    <>
      ...
      <div className="fixed inset-0 z-[55] flex items-center justify-center p-4 pointer-events-none">
        <div className="relative w-full max-w-lg pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            onClick={onClose}
            className="absolute top-2 right-2 p-1.5 rounded-full bg-violet-800/80 hover:bg-violet-700 text-violet-200 hover:text-white transition-colors cursor-pointer z-10"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
  ...
  return (
    <>
      ...
      <tr
        ...
        className="bg-violet-900/80"
        data-date={group.dateISO}
      >
        <td className="border-b border-violet-800/70"></td>
        <td
          colSpan={COLUMN_COUNT - 1}
          className="px-3 py-1.5 text-xs font-semibold text-cyan-400/80 uppercase tracking-wider border-b border-violet-800/70"
        >
          {group.label}
        </td>
      </tr>
      ...
      {group.events.map((event) => {
        ...
        return (
          <tr
            key={event.id}
            className={`hover:bg-violet-900/70 transition-colors cursor-pointer ${event.isDuplicate ? 'bg-red-950/30' : event.isFeatured ? 'bg-cyan-500/5 border-l-2 border-l-cyan-500/40' : 'bg-violet-950'}`}
          ...
          >
            ...
            <td className="px-2 py-2">
              {(() => {
                ...
                return (
                  ...
                  >
                    <Star
                      className={`w-4 h-4 ${isInItinerary ? 'text-yellow-400 fill-yellow-400' : 'text-violet-500 hover:text-violet-300'}`}
                    />
                    {fc > 0 && (
                      <span className="absolute -top-1 -right-1.5 min-w-[14px] h-[14px] flex items-center justify-center rounded-full bg-cyan-500 text-white text-[8px] font-bold px-0.5 pointer-events-none">
                        {fc}
                      </span>
                    )}
                  </button>
            ...
            {/* Time */}
            <td className="px-3 py-2 text-violet-300 whitespace-nowrap">
            ...
            <td className="px-3 py-2 text-violet-300 truncate hidden sm:table-cell" title={event.organizer}>
              {event.organizer}
            </td>

            {/* Event Name (+ organizer on mobile portrait) */}
            <td className="px-3 py-2 font-medium text-violet-100 overflow-hidden truncate max-w-[25ch] sm:max-w-none" title={event.name}>
              ...
              {event.organizer && (
                <div className="sm:hidden text-violet-300 text-xs font-normal truncate mt-0.5">
                  {event.organizer}
                </div>
              )}
            </td>
            ...
            {/* Location */}
            <td className="px-3 py-2 text-violet-300 truncate max-w-[20ch] sm:max-w-none" title={event.address}>
              {event.address ? (
