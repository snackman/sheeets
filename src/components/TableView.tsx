    ...
    <>
      ...
      {group.events.map((event) => {
        ...
        return (
          <tr
            key={event.id}
            className={`hover:bg-stone-900/70 transition-colors cursor-pointer ${event.isDuplicate ? 'bg-red-950/30' : event.isFeatured ? 'bg-amber-500/5 border-l-2 border-l-orange-500/40' : 'bg-stone-950'}`}
