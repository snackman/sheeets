  ...
  return (
    <div className="max-w-full mx-auto px-2 sm:px-4 pb-3 flex-1 min-h-0 min-w-0 flex flex-col w-full">
      <div
        ...
        className="overflow-auto rounded-lg border border-sky-800 flex-1 min-h-0 min-w-0 overscroll-none"
      >
        <table className="w-full text-sm text-left md:table-fixed" style={{ minWidth: Math.max(640, 142 + tagsColWidth + 420) }}>
          ...
          <thead className="text-xs uppercase tracking-wider text-sky-400 bg-sky-900 border-b border-sky-800 sticky top-0 z-20">
            <tr>
              ...
              <th className="px-3 py-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={downloadCSV}
                      className="p-1 rounded text-sky-500 hover:text-sky-200 transition-colors cursor-pointer"
                      title="Download CSV"
                    ...
                    <a
                      ...
                      className="p-1 rounded text-sky-500 hover:text-sky-200 transition-colors"
    ...
    <>
      ...
      <div className="fixed inset-0 z-[55] flex items-center justify-center p-4 pointer-events-none">
        <div className="relative w-full max-w-lg pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            onClick={onClose}
            className="absolute top-2 right-2 p-1.5 rounded-full bg-sky-800/80 hover:bg-sky-700 text-sky-300 hover:text-white transition-colors cursor-pointer z-10"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
  ...
  return (
    <>
      ...
      <tr
        ...
        className="bg-sky-900/80"
        data-date={group.dateISO}
      >
        <td className="border-b border-sky-800/70"></td>
        <td
          colSpan={COLUMN_COUNT - 1}
          className="px-3 py-1.5 text-xs font-semibold text-teal-400/80 uppercase tracking-wider border-b border-sky-800/70"
        >
          {group.label}
        </td>
      </tr>
      ...
      {group.events.map((event) => {
        ...
        return (
          <tr
            key={event.id}
            className={`hover:bg-sky-900/70 transition-colors cursor-pointer ${event.isDuplicate ? 'bg-red-950/30' : event.isFeatured ? 'bg-teal-500/5 border-l-2 border-l-teal-500/40' : 'bg-sky-950'}`}
          ...
          >
            ...
            <td className="px-2 py-2">
              {(() => {
                ...
                return (
                  ...
                  >
                    <Star
                      className={`w-4 h-4 ${isInItinerary ? 'text-yellow-400 fill-yellow-400' : 'text-sky-600 hover:text-sky-400'}`}
                    />
                    {fc > 0 && (
                      <span className="absolute -top-1 -right-1.5 min-w-[14px] h-[14px] flex items-center justify-center rounded-full bg-teal-500 text-white text-[8px] font-bold px-0.5 pointer-events-none">
                        {fc}
                      </span>
                    )}
                  </button>
            ...
            {/* Time */}
            <td className="px-3 py-2 text-sky-400 whitespace-nowrap">
            ...
            <td className="px-3 py-2 text-sky-400 truncate hidden sm:table-cell" title={event.organizer}>
              {event.organizer}
            </td>

            {/* Event Name (+ organizer on mobile portrait) */}
            <td className="px-3 py-2 font-medium text-sky-100 overflow-hidden truncate max-w-[25ch] sm:max-w-none" title={event.name}>
              ...
              {event.organizer && (
                <div className="sm:hidden text-sky-400 text-xs font-normal truncate mt-0.5">
                  {event.organizer}
                </div>
              )}
            </td>
            ...
            {/* Location */}
            <td className="px-3 py-2 text-sky-400 truncate max-w-[20ch] sm:max-w-none" title={event.address}>
              {event.address ? (
