...
function FriendsGoingModal({
  ...
  return createPortal(
    ...
    >
      ...
      {/* Modal */}
      <div
        className="relative bg-blue-900 border border-blue-700 rounded-xl shadow-2xl w-full max-w-sm"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-blue-700">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-blue-400 truncate">{eventName}</p>
          </div>
          <button
            onClick={onClose}
            className="p-1 text-blue-400 hover:text-white transition-colors cursor-pointer shrink-0 ml-2"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="overflow-y-auto max-h-[60vh] p-3 space-y-2">
          {friends.map((friend) => (
            <div
              key={friend.userId}
              className="flex items-center gap-3 px-3 py-2 rounded-lg bg-blue-800 hover:bg-blue-800 transition-colors"
            >
...
export function EventCard({
  ...
  return (
    <div className={`rounded-lg p-4 transition-colors group flex gap-4 overflow-hidden ${
      event.isFeatured
        ? 'bg-blue-900 border-2 border-yellow-500/50 hover:bg-blue-800 hover:border-yellow-500/70 active:bg-blue-800 active:border-yellow-500/70'
        : 'bg-blue-900 border border-blue-700 hover:bg-blue-800 hover:border-blue-600 active:bg-blue-800 active:border-blue-600'
    }`}>
      {/* Left: cover image */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-white text-sm sm:text-base leading-tight">
              {event.link ? (
                <a
                  ...
                  className="hover:text-yellow-400 active:text-yellow-400 transition-colors"
            ...
            {event.organizer && (
              <p className="text-blue-500 text-xs mt-0.5">{event.organizer}</p>
            )}
          </div>
          ...
          <div className="flex flex-col items-center shrink-0">
            ...
            {event.link && (
              <button
                ...
                className="p-1 text-blue-500 hover:text-blue-300 transition-colors cursor-pointer"
        ...
        {event.address && (
          <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
            ...
            className="w-full text-blue-500 hover:text-blue-300 text-sm mt-1 flex items-start gap-1 overflow-hidden transition-colors min-w-0">
        ...
        {friendsGoing && friendsGoing.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 mt-2 px-2 py-1.5 -mx-1 rounded-lg hover:bg-blue-800/50 transition-colors cursor-pointer group/friends w-fit"
          >
        ...
        {checkedInFriends && checkedInFriends.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 mt-1 px-2 py-1.5 -mx-1 rounded-lg hover:bg-blue-800/50 transition-colors cursor-pointer group/checkin w-fit"
          >
        ...
        {/* Note */}
        {event.note && (
          <p className="text-blue-600 text-xs mt-1 italic truncate">{event.note}</p>
        )}
