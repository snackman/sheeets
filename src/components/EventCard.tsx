...
function FriendsGoingModal({
  ...
  return createPortal(
    ...
    >
      ...
      {/* Modal */}
      <div
        className="relative bg-violet-900 border border-violet-800 rounded-xl shadow-2xl w-full max-w-sm"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-violet-800">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-violet-300 truncate">{eventName}</p>
          </div>
          <button
            onClick={onClose}
            className="p-1 text-violet-300 hover:text-white transition-colors cursor-pointer shrink-0 ml-2"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="overflow-y-auto max-h-[60vh] p-3 space-y-2">
          {friends.map((friend) => (
            <div
              key={friend.userId}
              className="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-750 hover:bg-violet-800 transition-colors"
            >
...
export function EventCard({
  ...
  return (
    <div className={`rounded-lg p-4 transition-colors group flex gap-4 overflow-hidden ${
      event.isFeatured
        ? 'bg-violet-900 border-2 border-cyan-500/50 hover:bg-slate-750 hover:border-cyan-500/70 active:bg-slate-750 active:border-cyan-500/70'
        : 'bg-violet-900 border border-violet-800 hover:bg-slate-750 hover:border-violet-700 active:bg-slate-750 active:border-violet-700'
    }`}>
      {/* Left: cover image */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-white text-sm sm:text-base leading-tight">
              {event.link ? (
                <a
                  ...
                  className="hover:text-cyan-400 active:text-cyan-400 transition-colors"
            ...
            {event.organizer && (
              <p className="text-violet-400 text-xs mt-0.5">{event.organizer}</p>
            )}
          </div>
          ...
          <div className="flex flex-col items-center shrink-0">
            ...
            {event.link && (
              <button
                ...
                className="p-1 text-violet-400 hover:text-violet-200 transition-colors cursor-pointer"
        ...
        <div className="relative w-fit mt-1">
          <p className="text-violet-300 text-sm flex items-start gap-1">
        ...
        {event.address && (
          <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
            ...
            className="w-full text-violet-400 hover:text-violet-200 text-sm mt-1 flex items-start gap-1 overflow-hidden transition-colors min-w-0">
        ...
        {friendsGoing && friendsGoing.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 mt-2 px-2 py-1.5 -mx-1 rounded-lg hover:bg-violet-800/50 transition-colors cursor-pointer group/friends w-fit"
          >
        ...
        {checkedInFriends && checkedInFriends.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 mt-1 px-2 py-1.5 -mx-1 rounded-lg hover:bg-violet-800/50 transition-colors cursor-pointer group/checkin w-fit"
          >
        ...
        {/* Note */}
        {event.note && (
          <p className="text-violet-500 text-xs mt-1 italic truncate">{event.note}</p>
        )}
