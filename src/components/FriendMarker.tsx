      ...
      >
        ...
        <div
          className={`w-8 h-8 rounded-full border-2 shadow-lg flex items-center justify-center overflow-hidden ${
            isRecent ? 'border-green-400' : 'border-sky-400'
          }`}
        ...
        {isRecent && (
          <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5">
            ...
            <div className="absolute inset-0 w-2.5 h-2.5 bg-green-400 rounded-full border border-sky-950" />
          </div>
        )}

        {/* Name + time label */}
        {showLabel && (
          <div className="mt-1 px-1.5 py-0.5 rounded bg-sky-900/90 text-[9px] text-white whitespace-nowrap max-w-[100px] truncate text-center leading-tight">
            <div className="font-medium truncate">{name.split(' ')[0]}</div>
            <div className="text-sky-400">{timeAgo(location.updated_at)}</div>
          </div>
        )}
      </div>
    </Marker>
