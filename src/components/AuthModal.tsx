  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
        <div className="bg-blue-900 border border-blue-700 rounded-xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-blue-700">
            ...
            <button onClick={handleClose} className="p-1 text-blue-400 hover:text-white transition-colors cursor-pointer">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="px-4 py-5">
            {step === 'email' && (
              <form onSubmit={handleEmailSubmit}>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full mt-4 px-4 py-2.5 bg-yellow-500 hover:bg-yellow-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                >
                ...
                <div className="flex justify-center gap-2" onPaste={handlePaste}>
                  {code.map((digit, i) => (
                    <input
                      ...
                      className="w-10 h-12 bg-blue-950 border border-blue-600 rounded-lg text-white text-center text-lg font-bold outline-none focus:border-yellow-500 transition-colors"
                    />
                  ))}
                </div>
                {error && <p className="text-red-400 text-xs mt-3 text-center">{error}</p>}
                {loading && <p className="text-yellow-400 text-xs mt-3 text-center">Verifying...</p>}
                <button
                  onClick={() => { setStep('email'); setError(''); }}
                  className="w-full mt-4 text-blue-400 hover:text-blue-300 text-xs text-center cursor-pointer"
                >
            ...
            {step === 'success' && (
              <div className="text-center py-4">
                ...
                <p className="text-blue-400 text-sm mt-1">Your itinerary will sync across devices.</p>
              </div>
            )}
          </div>
        </div>
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        {secondary && result.display_name && (
          <p className="text-xs text-blue-500 truncate">{secondary}</p>
        )}
      </div>
      {result.request_status === 'pending_outgoing' ? (
        <span className="flex items-center gap-1 text-xs text-blue-500 bg-blue-800 px-2.5 py-1.5 rounded-lg">
        ...
        <button
          ...
          disabled={sending === result.user_id}
          className="flex items-center gap-1 text-xs font-medium bg-yellow-500 hover:bg-yellow-600 disabled:opacity-50 text-white px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        {profile?.display_name && profile?.email && (
          <p className="text-xs text-blue-500 truncate">{profile.email}</p>
        )}
      </div>
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <button
          onClick={onReject}
          disabled={responding}
          className="flex items-center gap-1 text-xs font-medium border border-blue-600 hover:border-red-500/50 hover:bg-red-500/10 text-blue-400 hover:text-red-400 disabled:opacity-50 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
          <X className="w-3 h-3" />
        </button>
      </div>
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          onClick={onCancel}
          disabled={cancelling}
          className="p-1 text-blue-500 hover:text-red-400 disabled:opacity-50 transition-colors cursor-pointer"
  ...
  return (
    <>
      <button
        ...
        className="relative p-1.5 text-blue-400 hover:text-white transition-colors cursor-pointer"
        title="Settings"
      >
        <Settings className="w-4 h-4" />
        {badgeCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] flex items-center justify-center text-[9px] font-bold rounded-full bg-yellow-500 text-white px-0.5">
            {badgeCount}
          </span>
        )}
      </button>
            ...
            <div className="bg-blue-900 border border-blue-700 rounded-xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="max-h-[80vh] overflow-y-auto px-4 py-5 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex items-center bg-blue-950 border border-blue-600 rounded-lg focus-within:border-yellow-500">
                      <span className="text-blue-500 text-sm pl-3 select-none">@</span>
                      <input
                        type="text"
                        value={xHandle}
                        onChange={(e) => setXHandle(e.target.value.replace(/^@/, ''))}
                        placeholder="X handle"
                        className="flex-1 bg-transparent text-white text-sm px-2 py-2 focus:outline-none placeholder:text-blue-500"
                      />
                    </div>
                  </div>
                ...
                <div className="border-t border-blue-700 pt-4 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    {friendCount > 0 && (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm bg-blue-800 hover:bg-blue-700 text-white transition-colors cursor-pointer"
                      >
                  ...
                  {/* Search for friends */}
                  <div className="flex items-center gap-2 bg-blue-950 border border-blue-600 rounded-lg px-3 py-2 focus-within:border-yellow-500 transition-colors">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent text-white text-sm outline-none placeholder:text-blue-500"
                    />
                    {searchLoading && <Loader2 className="w-4 h-4 text-blue-500 animate-spin shrink-0" />}
                  </div>
                ...
                <div className="border-t border-blue-700 pt-4 space-y-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-800 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-blue-600 hover:border-red-500/50 hover:bg-red-500/10 text-blue-400 hover:text-red-400 rounded-lg text-sm transition-colors cursor-pointer"
                  >
