  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
        <div className="bg-sky-900 border border-sky-800 rounded-xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-sky-800">
            ...
            <button onClick={handleClose} className="p-1 text-sky-400 hover:text-white transition-colors cursor-pointer">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="px-4 py-5">
            {step === 'email' && (
              <form onSubmit={handleEmailSubmit}>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full mt-4 px-4 py-2.5 bg-teal-500 hover:bg-teal-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                >
                ...
                <div className="flex justify-center gap-2" onPaste={handlePaste}>
                  {code.map((digit, i) => (
                    <input
                      ...
                      className="w-10 h-12 bg-sky-950 border border-sky-700 rounded-lg text-white text-center text-lg font-bold outline-none focus:border-teal-500 transition-colors"
                    />
                  ))}
                </div>
                {error && <p className="text-red-400 text-xs mt-3 text-center">{error}</p>}
                {loading && <p className="text-teal-400 text-xs mt-3 text-center">Verifying...</p>}
                <button
                  onClick={() => { setStep('email'); setError(''); }}
                  className="w-full mt-4 text-sky-400 hover:text-sky-300 text-xs text-center cursor-pointer"
                >
            ...
            {step === 'success' && (
              <div className="text-center py-4">
                ...
                <p className="text-sky-400 text-sm mt-1">Your itinerary will sync across devices.</p>
              </div>
            )}
          </div>
        </div>
...
function SearchResultRow({
  ...
  return (
    <div className="flex items-center gap-3 py-2">
      <div className="w-8 h-8 rounded-full bg-sky-800 flex items-center justify-center shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        {secondary && result.display_name && (
          <p className="text-xs text-sky-500 truncate">{secondary}</p>
        )}
      </div>
      {result.request_status === 'pending_outgoing' ? (
        <span className="flex items-center gap-1 text-xs text-sky-500 bg-sky-800 px-2.5 py-1.5 rounded-lg">
        ...
        <button
          ...
          disabled={sending === result.user_id}
          className="flex items-center gap-1 text-xs font-medium bg-teal-500 hover:bg-teal-600 disabled:opacity-50 text-white px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        {profile?.display_name && profile?.email && (
          <p className="text-xs text-sky-500 truncate">{profile.email}</p>
        )}
      </div>
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <button
          onClick={onReject}
          disabled={responding}
          className="flex items-center gap-1 text-xs font-medium border border-sky-700 hover:border-red-500/50 hover:bg-red-500/10 text-sky-400 hover:text-red-400 disabled:opacity-50 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
          <X className="w-3 h-3" />
        </button>
      </div>
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          onClick={onCancel}
          disabled={cancelling}
          className="p-1 text-sky-500 hover:text-red-400 disabled:opacity-50 transition-colors cursor-pointer"
  ...
  return (
    <>
      <button
        ...
        className="relative p-1.5 text-sky-400 hover:text-white transition-colors cursor-pointer"
        title="Settings"
      >
        <Menu className="w-4 h-4" />
        {badgeCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] flex items-center justify-center text-[9px] font-bold rounded-full bg-teal-500 text-white px-0.5">
            {badgeCount}
          </span>
        )}
      </button>
            ...
            <div className="bg-sky-900 border border-sky-800 rounded-xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="max-h-[80vh] overflow-y-auto px-4 py-5 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex items-center bg-sky-950 border border-sky-700 rounded-lg focus-within:border-teal-500">
                      <span className="text-sky-500 text-sm pl-3 select-none">@</span>
                      <input
                        type="text"
                        value={xHandle}
                        onChange={(e) => setXHandle(e.target.value.replace(/^@/, ''))}
                        placeholder="X handle"
                        className="flex-1 bg-transparent text-white text-sm px-2 py-2 focus:outline-none placeholder:text-sky-500"
                      />
                    </div>
                  </div>
                ...
                {/* Friends */}
                <div className="border-t border-sky-800 pt-4 space-y-3">
                  {/* Friends button + Friend Link button */}
                  <div className="flex items-center gap-2">
                    {friendCount > 0 && (
                      <button
                        onClick={() => { setOpen(false); onOpenFriends(); }}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm bg-sky-800 hover:bg-sky-700 text-white transition-colors cursor-pointer"
                      >
                  ...
                  {/* Search for friends */}
                  <div className="flex items-center gap-2 bg-sky-950 border border-sky-700 rounded-lg px-3 py-2 focus-within:border-teal-500 transition-colors">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent text-white text-sm outline-none placeholder:text-sky-500"
                    />
                    {searchLoading && <Loader2 className="w-4 h-4 text-sky-500 animate-spin shrink-0" />}
                  </div>
                  ...
                  {searchQuery.trim() && !searchLoading && searchResults.length === 0 && (
                    <p className="text-xs text-sky-500 text-center py-2">
                      No users found
                    </p>
                  )}
                  ...
                  {outgoingRequests.length > 0 && (
                    <div>
                      <button
                        ...
                        className="text-xs text-sky-500 hover:text-sky-400 transition-colors cursor-pointer"
                      >
                ...
                <div className="border-t border-sky-800 pt-4 space-y-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-sky-800 hover:bg-sky-700 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-sky-700 hover:border-red-500/50 hover:bg-red-500/10 text-sky-400 hover:text-red-400 rounded-lg text-sm transition-colors cursor-pointer"
                  >
