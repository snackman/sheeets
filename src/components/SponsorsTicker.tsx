...
export function SponsorsTicker({ sponsors }: SponsorsTickerProps) {
  ...
  const item = (
    <span className="inline-flex items-center">
      {sponsorList.map((s, i) => (
        <span key={i}>
          {i > 0 && <span className="mx-6 text-blue-600">&#10022;</span>}
          {s.beforeText} <a
            href={s.url}
            target="_blank"
            rel="noopener noreferrer"
            className="underline decoration-blue-500 underline-offset-2 hover:text-blue-200 hover:decoration-blue-300 transition-colors"
          >{s.linkText}</a>{s.afterText}
        </span>
      ))}
      <span className="mx-6 text-blue-600">&#10022;</span>
    </span>
  );

  return (
    <div className="w-full overflow-hidden bg-blue-950/80 border-b border-blue-800/50 py-1.5">
      <div className="sponsors-scroll inline-flex whitespace-nowrap text-xs text-blue-400">
