...
export function POISearchBar({ onAddPOI, mapRef }: POISearchBarProps) {
    ...
    return (
      <div className="absolute top-2 left-12 z-10">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-sky-900 border border-sky-700 rounded-lg text-xs text-sky-300 hover:text-white hover:bg-sky-800 active:bg-sky-800 transition-colors cursor-pointer shadow-lg"
        >
  ...
  return (
    <div
      ref={containerRef}
      className="absolute top-2 left-12 z-10 w-72 bg-sky-900 border border-sky-800 rounded-lg shadow-xl"
    >
        ...
        <div className="relative">
          <div className="flex items-center gap-2 px-3 py-2">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-sm text-white placeholder:text-sky-500 outline-none"
            />
            {loading && <Loader2 className="w-4 h-4 text-sky-400 animate-spin shrink-0" />}
            <button
              onClick={handleClose}
              className="p-0.5 text-sky-400 hover:text-white transition-colors"
          ...
          {/* Results dropdown */}
          {results.length > 0 && (
            <div className="border-t border-sky-800 max-h-52 overflow-y-auto">
              {results.map((result, i) => (
                <button
                  key={i}
                  onClick={() => handleSelectResult(result)}
                  className="w-full text-left px-3 py-2 hover:bg-sky-800 active:bg-sky-800 transition-colors cursor-pointer"
                >
                  <p className="text-sm text-white truncate">{result.text}</p>
                  <p className="text-xs text-sky-400 truncate">{result.place_name}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="p-3 space-y-3">
          ...
          <div>
            <input
              ...
              className="w-full bg-sky-800 border border-sky-700 rounded px-2.5 py-1.5 text-sm text-white placeholder:text-sky-500 outline-none focus:border-sky-500"
              maxLength={60}
            />
            <p className="text-xs text-sky-500 mt-1 truncate">{selectedResult.place_name}</p>
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-center w-8 h-8 rounded-lg border transition-colors cursor-pointer ${
                    isActive
                      ? 'border-white/30 bg-sky-700'
                      : 'border-sky-700 bg-sky-800 hover:bg-sky-700'
                  }`}
                  title={cat.label}
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-sky-400">Share with friends</span>
            <button
              type="button"
              onClick={() => setIsPublic(v => !v)}
              className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer ${
                isPublic ? 'bg-teal-500' : 'bg-sky-700'
              }`}
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-500 hover:bg-teal-600 active:bg-teal-600 disabled:opacity-50 text-white text-xs font-medium rounded-lg transition-colors cursor-pointer"
            >
              {saving ? (
            ...
            <button
              onClick={handleClose}
              className="px-3 py-1.5 text-sky-400 hover:text-white text-xs transition-colors cursor-pointer"
            >
              Cancel
            </button>
          </div>
