...
export function OGImage({ url, eventId }: OGImageProps) {
  ...
  return (
    <div ref={ref} className="shrink-0 w-[88px] sm:w-[106px] rounded-lg overflow-hidden bg-violet-800/30 self-center">
      {imageUrl && (
        <img
          src={imageUrl}
          alt=""
      ...
      {!loaded && (
        <div className="w-full h-[88px] sm:h-[106px] animate-pulse bg-violet-800/50" />
      )}
    </div>
  );
}
