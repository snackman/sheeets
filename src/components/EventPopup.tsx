      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start gap-1">
          <div className="flex-1 min-w-0">
            ...
            {event.organizer && (
              <p className="text-xs text-blue-500 mt-0.5">{event.organizer}</p>
            )}
          </div>
          {onItineraryToggle && (
            <StarButton
        ...
        {event.address && (
          <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
            ...
            className="w-full text-blue-500 hover:text-blue-300 text-xs mt-1 flex items-center gap-1 overflow-hidden transition-colors">
        ...
        {/* Note */}
        {event.note && (
          <p className="text-blue-600 text-xs mt-1 italic line-clamp-2">{event.note}</p>
        )}
        ...
        <div className="max-h-[260px] overflow-y-auto space-y-2 pr-1">
          {events.map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="flex gap-2.5 p-2.5 bg-blue-800/50 hover:bg-blue-700/50 active:bg-blue-700/50 rounded-lg transition-colors cursor-pointer"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start gap-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {event.organizer && (
                        <p className="text-[10px] text-blue-500 mt-0.5 truncate">{event.organizer}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {event.address && (
                    <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
                      eventId={event.id} eventName={event.name}
                      className="w-full text-blue-500 hover:text-blue-300 text-[10px] mt-0.5 flex items-center gap-1 overflow-hidden transition-colors">
