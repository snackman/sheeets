      ...
      <div className="w-[220px]">
        ...
        {poi.address && (
          <AddressLink
            ...
            className="text-sky-500 hover:text-sky-300 text-xs mt-1.5 flex items-center gap-1 transition-colors"
          >
        ...
        {/* Note */}
        {poi.note && (
          <p className="text-sky-400 text-xs mt-1.5 italic line-clamp-2">{poi.note}</p>
        )}

        {/* Friend attribution */}
        {!isOwn && ownerName && (
          <p className="text-xs text-sky-500 mt-1">Shared by {ownerName}</p>
        )}
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer ${
                  poi.is_public ? 'bg-teal-500' : 'bg-sky-700'
                }`}
              >
