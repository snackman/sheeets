...
function FriendCard({
  ...
  return (
    <div className="bg-[var(--theme-bg-secondary)] rounded-lg border border-[var(--theme-border-primary)] overflow-hidden">
      {/* Tappable header */}
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {friend.display_name && friend.email && (
            <p className="text-xs text-[var(--theme-text-muted)] truncate">{friend.email}</p>
          )}
        </div>

        <ChevronRight
          className={`w-4 h-4 text-[var(--theme-text-muted)] shrink-0 transition-transform ${expanded ? 'rotate-90' : ''}`}
        />
      </button>

      {/* Expanded profile */}
      {expanded && (
        <div className="px-3 pb-3 pt-1 border-t border-[var(--theme-border-primary)]/50 space-y-2">
          {friend.email && (
          ...
          {friend.x_handle && (
            <a
              ...
              className="flex items-center gap-1.5 text-xs text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)] transition-colors"
            >
          ...
          {!friend.email && !hasSocials && (
            <p className="text-xs text-[var(--theme-text-muted)]">No profile info yet</p>
          )}
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-[var(--theme-text-muted)] hover:text-red-400 transition-colors cursor-pointer mt-1"
          >
...
export function FriendsPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 z-[70] h-full w-full max-w-sm bg-[var(--theme-bg-primary)] border-l border-[var(--theme-border-primary)] shadow-2xl transition-transform duration-300 ease-in-out pt-[var(--safe-area-top)] ${
        ...
        <div className="overflow-y-auto h-[calc(100%-57px)] px-4 pb-4">
          {friends.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Users className="w-12 h-12 text-[var(--theme-text-faint)] mb-4" />
              <p className="text-[var(--theme-text-secondary)] font-medium mb-2">No friends yet</p>
