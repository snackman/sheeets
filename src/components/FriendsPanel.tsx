      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {friend.display_name && friend.email && (
            <p className="text-xs text-stone-500 truncate">{friend.email}</p>
          )}
        </div>

        <ChevronRight
          className={`w-4 h-4 text-stone-500 shrink-0 transition-transform ${expanded ? 'rotate-90' : ''}`}
        />
      </button>
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-1 border-t border-stone-700/50 space-y-2">
          ...
          {!friend.email && !hasSocials && (
            <p className="text-xs text-stone-500">No profile info yet</p>
          )}
...
export function FriendsPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 z-[70] h-full w-full max-w-sm bg-stone-950 border-l border-stone-700 shadow-2xl transition-transform duration-300 ease-in-out pt-[var(--safe-area-top)] ${
        ...
        <div className="overflow-y-auto h-[calc(100%-57px)] px-4 pb-4">
          {friends.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Users className="w-12 h-12 text-stone-600 mb-4" />
              <p className="text-stone-400 font-medium mb-2">No friends yet</p>
