...
export function EventApp({ initialConference }: { initialConference?: string }) {
    ...
    return (
      <div className="min-h-screen bg-blue-950">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 px-4">
          ...
          <p className="text-blue-500 text-sm text-center max-w-md">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-2 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 active:bg-yellow-600 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="h-dvh flex flex-col bg-blue-950 overflow-hidden">
      <Header
        viewMode={viewMode}
        onViewChange={setViewMode}
