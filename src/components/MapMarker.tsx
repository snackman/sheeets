...
function ClockPin({
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={cx} cy={cy} r={ir} fill="#0c4a6e" />
      ...
      <div className="relative group" style={{ width: 0, height: 0 }}>
        ...
        >
          <div className="relative flex items-center justify-center">
            {/* Clock-face pin */}
            <div className={`rounded-full ${isFeatured ? 'ring-2 ring-teal-500/60' : ''}`}>
              <ClockPin
        ...
        {showLabel && (
          ...
          >
            <div className={`px-2 py-0.5 rounded text-[10px] text-white max-w-[140px] leading-tight transition-colors ${
              isFeatured
                ? 'bg-teal-500/20 ring-1 ring-teal-500/40 group-hover:bg-teal-500/30'
                : 'bg-sky-900/90 group-hover:bg-sky-800/90'
            }`}>
              <div className="truncate whitespace-nowrap">{label}</div>
              {showOrganizer && organizer && (
                <div className="truncate whitespace-nowrap text-sky-400">{organizer}</div>
              )}
