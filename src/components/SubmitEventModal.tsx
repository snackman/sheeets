  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
        <div
          className="bg-violet-900 border border-violet-800 rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-violet-800 shrink-0">
          ...
          <div className="px-4 py-5 overflow-y-auto flex-1">
            ...
            {step === 'input' && (
              <div className="space-y-4">
                ...
                {fetchLoading ? (
                  <div className="flex items-center justify-center gap-2 py-2.5 text-violet-300 text-sm">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 bg-cyan-500 hover:bg-cyan-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer flex items-center justify-center gap-2"
                  >
                    Fetch Event
                  </button>
                )}
                ...
                <button
                  ...
                  className="w-full text-violet-300 hover:text-violet-200 text-xs text-center cursor-pointer"
                >
            ...
            {step === 'form' && (
              <div className="space-y-4">
                {/* Conference selector */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Conference</label>
                  <select
                    value={conference}
                    onChange={(e) => setConference(e.target.value)}
                    className="w-full bg-violet-950 border border-violet-700 rounded-lg text-white text-sm px-3 py-2 focus:border-cyan-500 focus:outline-none"
                  >
                    {EVENT_TABS.map((t) => (
                ...
                <div className="flex items-end gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="bg-violet-950 border border-violet-700 rounded-lg text-white text-sm px-3 py-1.5 focus:border-cyan-500 focus:outline-none cursor-pointer [&::-webkit-calendar-picker-indicator]:invert"
                      style={{ colorScheme: 'dark' }}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-violet-300 mb-1">Start</label>
                    <Dropdown
                      value={startTime24 || ''}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-violet-300 mb-1">End</label>
                    <Dropdown
                      value={endTime24 || ''}
                      options={['', ...TIME_OPTIONS]}
                      renderOption={(v) => v ? format12Hour(v) : '—'}
                ...
                {/* Organizer */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Organizer</label>
                  <input
                    type="text"
                    value={organizer}
                ...
                {/* Address */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Address</label>
                  <AddressAutocomplete value={address} onChange={setAddress} />
                </div>

                {/* Cost */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Cost</label>
                  <input
                    type="text"
                    value={cost}
                ...
                {/* Link */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Link</label>
                  <input
                    type="url"
                    value={link}
                ...
                {/* Tags */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1.5">Tags</label>
                ...
                <div className="flex items-center gap-6">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded border-violet-700 bg-violet-950 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0 cursor-pointer"
                    />
                    <span className="text-sm text-violet-200">Food</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 rounded border-violet-700 bg-violet-950 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0 cursor-pointer"
                    />
                    <span className="text-sm text-violet-200">Bar</span>
                  </label>
                </div>

                {/* Note */}
                <div>
                  <label className="block text-xs font-medium text-violet-300 mb-1">Note</label>
                  <textarea
                    value={note}
                    onChange={(e) => setNote(e.target.value)}
                    placeholder="Any additional details..."
                    rows={2}
                    className="w-full bg-violet-950 border border-violet-700 rounded-lg text-white text-sm px-3 py-2 focus:border-cyan-500 focus:outline-none placeholder:text-violet-400 resize-none"
                  />
                </div>
            ...
            {step === 'success' && (
              <div className="text-center py-4">
                ...
                {upsellCopy && (
                  <div className="mt-4 p-4 rounded-xl bg-gradient-to-br from-cyan-500/10 to-violet-500/10 border border-cyan-500/30 text-left">
                    <h4 className="text-sm font-semibold text-cyan-300 mb-1">{upsellCopy.heading}</h4>
                    <p className="text-xs text-violet-300 mb-3">{upsellCopy.body}</p>
                ...
                <button
                  onClick={handleClose}
                  className="mt-4 px-4 py-2 text-sm text-violet-300 hover:text-white transition-colors cursor-pointer"
                >
                  Close
                </button>
              </div>
          ...
          {/* Footer */}
          {step !== 'success' && (
            <div className="px-4 py-3 border-t border-violet-800 shrink-0">
              <a
                href={sheetUrl}
                target="_blank"
