...
const inputClass = 'bg-sky-800 border border-sky-700 rounded-lg px-3 py-2 text-white text-sm w-full focus:border-blue-500 focus:outline-none';
...
export default function AdminPage() {
    ...
    return (
      <div className="min-h-screen bg-sky-950 flex items-center justify-center p-4">
        <form
          ...
          className="bg-sky-900 border border-sky-800 rounded-xl p-6 w-full max-w-sm"
        >
          ...
          <input
            ...
            className="w-full bg-sky-950 border border-sky-700 rounded-lg text-white text-sm px-3 py-2 focus:border-teal-500 focus:outline-none placeholder:text-sky-500 mb-3"
            autoFocus
          />
          <button
            type="submit"
            className="w-full px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
          >
            Enter
          </button>
        </form>
  ...
  return (
    <div className="min-h-screen bg-sky-950 text-white">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-sky-950 border-b border-sky-900 px-4 py-3">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <a href="/" className="text-sky-400 hover:text-white transition-colors">
          ...
          {/* Tab bar */}
          <div className="flex gap-1 bg-sky-900 rounded-lg p-1 mb-3">
            ...
            <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                      conference === tab.name
                        ...
                        : 'bg-sky-900 text-sky-400 hover:text-white'
                    }`}
                  >
                    {tab.name}
                  </button>
              ...
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full bg-sky-900 border border-sky-800 rounded-lg text-white text-sm pl-9 pr-3 py-1.5 focus:border-teal-500 focus:outline-none placeholder:text-sky-500"
                />
              </div>
            </div>
          )}
      ...
      <div className="max-w-5xl mx-auto px-4 py-4">
        ...
        {activeTab === 'featured' && (
          <>
            {loading ? (
              <div className="flex items-center justify-center py-20 gap-2 text-sky-400">
              ...
              <div className="space-y-1">
                {filtered.map((event) => (
                  ...
                  >
                    ...
                    {event.isFeatured && (
                      <span className="text-[10px] font-medium text-teal-400 bg-teal-500/10 px-2 py-0.5 rounded-full shrink-0">
                        FEATURED
                      </span>
                    )}
                  </div>
                ...
                {filtered.length === 0 && (
                  <p className="text-center text-sky-500 py-10">No events found</p>
                )}
              </div>
            )}
          </>
        ...
        {activeTab === 'sponsors' && (
          <div className="space-y-6">
            {configLoading ? (
              ...
              <>
                <div className="bg-sky-900 rounded-xl p-4 border border-sky-800">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-sm font-semibold text-white">Sponsors</h3>
                      <p className="text-xs text-sky-500 mt-0.5">Each sponsor shows as: before text + <span className="text-blue-400">link text</span> + after text</p>
                    </div>
                    <button
                      onClick={() => {
                  ...
                  {sponsors.length === 0 && (
                    <p className="text-sky-500 text-sm py-4 text-center">No sponsors added yet</p>
                  )}

                  <div className="space-y-3">
                    {sponsors.map((sponsor, idx) => (
                      <div key={idx} className="p-3 bg-sky-850 rounded-lg border border-sky-700">
                        {editingSponsorIndex === idx ? (
                          <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                              <div>
                                <label className="block text-xs text-sky-400 mb-1">Before Text</label>
                                <input
                                  type="text"
                              ...
                              <div>
                                <label className="block text-xs text-sky-400 mb-1">Link Text</label>
                                <input
                                  type="text"
                            ...
                            <div>
                              <label className="block text-xs text-sky-400 mb-1">After Text</label>
                              <input
                                type="text"
                            ...
                            <div>
                              <label className="block text-xs text-sky-400 mb-1">URL</label>
                              <input
                                type="url"
                                value={sponsor.url}
                                onChange={(e) => {
                            ...
                            {/* Inline preview */}
                            <div className="text-xs text-sky-500 bg-sky-950/50 rounded-lg px-3 py-2">
                              Preview: <span className="text-sky-300">{sponsor.beforeText}<span className="text-blue-400 underline">{sponsor.linkText}</span>{sponsor.afterText}</span>
                            </div>
                            <button
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-sky-500 truncate mt-0.5">{sponsor.url || '(no url)'}</p>
                            </div>
                            <button
                              onClick={() => setEditingSponsorIndex(idx)}
                              className="text-sky-400 hover:text-white cursor-pointer p-1"
                              title="Edit"
                            >
          ...
          <div className="space-y-6">
            {configLoading ? (
              ...
              <>
                ...
                {nativeAds.length === 0 && (
                  <p className="text-sky-500 text-sm py-4 text-center">No native ads configured</p>
                )}

                <div className="space-y-4">
                  {nativeAds.map((ad) => (
                    <div key={ad.id} className="bg-sky-900 rounded-xl p-4 border border-sky-800">
                      {editingAdId === ad.id ? (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-sky-500 font-mono">{ad.id.slice(0, 8)}...</span>
                            <button
                          ...
                          <div>
                            <label className="block text-xs text-sky-400 mb-1">Title</label>
                            <input
                              type="text"
                              value={ad.title}
                          ...
                          <div>
                            <label className="block text-xs text-sky-400 mb-1">Description</label>
                            <textarea
                          ...
                          <div>
                            <label className="block text-xs text-sky-400 mb-1">Link</label>
                            <input
                              type="url"
                              value={ad.link}
                          ...
                          <div>
                            <label className="block text-xs text-sky-400 mb-1">Image URL</label>
                            <input
                              type="url"
                              value={ad.imageUrl}
                          ...
                          <div className="flex gap-3">
                            <div className="flex-1">
                              <label className="block text-xs text-sky-400 mb-1">Conference</label>
                              <select
                                value={ad.conference}
                            ...
                            <div className="flex-1">
                              <label className="block text-xs text-sky-400 mb-1">Badge Text</label>
                              <input
                                type="text"
                                value={ad.badge}
                          ...
                          <div className="flex items-center gap-2">
                            <label className="flex items-center gap-2 cursor-pointer">
                              ...
                              />
                              <span className="text-sm text-sky-300">Active</span>
                            </label>
                          </div>
                        </div>
                      ) : (
                        <div className="flex items-start gap-4">
                          {ad.imageUrl && (
                            <div className="w-[80px] h-[60px] flex-shrink-0 rounded-lg overflow-hidden bg-sky-800">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-[10px] text-sky-500">{ad.active ? 'Active' : 'Inactive'}</span>
                            </div>
                            <p className="text-xs text-sky-400 line-clamp-1">{ad.description || '(no description)'}</p>
                            <p className="text-xs text-sky-500 mt-0.5">{ad.conference} &middot; {ad.badge || 'Sponsored'}</p>
                          </div>
        ...
        {activeTab === 'upsell' && (
          <div className="space-y-6">
            {configLoading ? (
              ...
              <>
                <div className="bg-sky-900 rounded-xl p-4 border border-sky-800 space-y-4">
                  <h3 className="text-sm font-semibold text-white">Upsell Copy (shown after event submission)</h3>

                  <div>
                    <label className="block text-xs text-sky-400 mb-1">Heading</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs text-sky-400 mb-1">Body</label>
                    <textarea
                  ...
                  <div className="flex gap-3">
                    <div className="flex-1">
                      <label className="block text-xs text-sky-400 mb-1">CTA Text</label>
                      <input
                        type="text"
                    ...
                    <div className="flex-1">
                      <label className="block text-xs text-sky-400 mb-1">CTA URL</label>
                      <input
                        type="url"
                ...
                <div className="bg-sky-900 rounded-xl p-4 border border-sky-800">
                  ...
                  <div className="max-w-md mx-auto">
                    <div className="p-4 rounded-xl bg-gradient-to-br from-teal-500/10 to-sky-500/10 border border-teal-500/30">
