...
export default function SharedItineraryPage() {
  ...
  if (notFound) {
    return (
      <div className="min-h-screen bg-violet-950 flex flex-col items-center justify-center px-4">
        <p className="text-violet-300 text-lg font-medium mb-2">Itinerary not found</p>
        <p className="text-violet-400 text-sm mb-4">This share link may have expired or is invalid.</p>
        <Link
          href="/"
  ...
  return (
    <div className={viewMode === 'map' ? 'h-screen flex flex-col bg-violet-950' : 'min-h-screen bg-violet-950'}>
      ...
      {sharedEvents.length > 0 && (
        <div className="bg-violet-900/50 border-b border-violet-900">
      ...
      {sharedEvents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center px-4">
          <Calendar className="w-12 h-12 text-violet-500 mb-4" />
          <p className="text-violet-300 font-medium mb-2">No matching events found</p>
              ...
              <div className="space-y-2">
                {group.events.map((event) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className="bg-violet-900 rounded-lg p-3 border border-violet-800"
                    >
                      ...
                      {event.organizer && (
                        <p className="text-violet-400 text-xs mt-0.5">By {event.organizer}</p>
                      )}

                      <p className="text-violet-300 text-xs mt-1">{timeDisplay}</p>

                      {event.address && (
                        <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
                          eventId={event.id} eventName={event.name}
                          className="text-violet-400 hover:text-violet-200 text-xs mt-0.5 truncate block transition-colors">
                          {event.address}
                        </AddressLink>
                      )}
