  ...
  () => import('@/components/MapView').then((mod) => ({ default: mod.MapView })),
  {
    ...
    loading: () => (
      <div className="w-full h-full bg-sky-950 flex items-center justify-center">
        <div className="text-sky-400">Loading map...</div>
      </div>
    ),
  }
);
...
export default function ItineraryPage() {
  ...
  const handleSharePNG = useCallback(async () => {
    ...
    try {
      ...
      const blob = await toBlob(captureRef.current, { backgroundColor: '#082f49', pixelRatio: 2 });
  ...
  return (
    <div className={viewMode === 'map' ? 'h-screen flex flex-col bg-sky-950' : 'min-h-screen bg-sky-950'}>
      {/* Header */}
      <header className="sticky top-0 z-50 bg-sky-950/95 backdrop-blur-sm border-b border-sky-900">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="p-1.5 text-sky-400 hover:text-white transition-colors"
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    activeConference === conf
                      ...
                      : 'bg-sky-900 text-sky-400 hover:text-sky-200 hover:bg-sky-800'
                  )}
                >
                  {conf.replace(/ 2026$/, '')}
                </button>
          ...
          <div className="flex items-center gap-1">
            {itineraryEvents.length > 0 && (
              <>
                {/* View toggle */}
                <div className="flex rounded-lg border border-sky-800 overflow-hidden mr-1">
                  {([
                ...
                <button
                  ...
                  className="p-1.5 text-sky-400 hover:text-teal-400 transition-colors cursor-pointer"
                ...
                <button
                  ...
                  disabled={exporting}
                  className="p-1.5 text-sky-400 hover:text-teal-400 transition-colors cursor-pointer disabled:opacity-50"
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    shareStatus === 'copied'
                      ...
                      : 'bg-sky-900 border border-sky-800 text-sky-400 hover:text-teal-400 hover:border-sky-700'
                  )}
                  title="Share link"
                >
      ...
      {itineraryEvents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center px-4">
          ...
          <p className="text-sky-400 font-medium mb-2">No events in your itinerary yet</p>
          <p className="text-sky-500 text-sm max-w-xs mb-4">
                      ...
                      >
                        ...
                        <div
                          className={`bg-sky-900 rounded-lg p-3 border transition-opacity ${
                            hasConflict ? 'border-amber-500/40' : 'border-sky-800'
                        ...
                        >
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <button
                              ...
                              className="shrink-0 p-1 text-sky-500 hover:text-red-400 transition-colors cursor-pointer"
                          ...
                          {event.organizer && (
                            <p className="text-sky-500 text-xs mt-0.5 ml-6">By {event.organizer}</p>
                          )}

                          <p className="text-sky-400 text-xs mt-1 ml-6">{timeDisplay}</p>

                          {event.address && (
                            <AddressLink address={event.address} navAddress={event.matchedAddress} lat={event.lat} lng={event.lng}
                              eventId={event.id} eventName={event.name}
                              className="text-sky-500 hover:text-sky-300 text-xs mt-0.5 truncate block transition-colors ml-6">
                              {event.address}
                            </AddressLink>
                          )}
          ...
          <div className="mt-6 pt-4 border-t border-sky-900">
            {showClearConfirm ? (
              <div className="flex items-center gap-2">
                <span className="text-sm text-sky-400">Clear all events?</span>
                <button
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-sky-800 hover:bg-sky-700 text-sky-300 text-xs font-medium rounded transition-colors cursor-pointer"
                >
                  Cancel
                </button>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-sky-900 hover:bg-sky-800 border border-sky-800 text-sky-400 hover:text-sky-300 rounded-lg text-sm font-medium transition-colors cursor-pointer"
              >
